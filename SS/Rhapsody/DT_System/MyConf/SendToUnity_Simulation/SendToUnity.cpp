...
//!	Generated Date	: Thu, 25, Jun 2020  
	File Path	: MyConf\SendToUnity_Simulation\SendToUnity.cpp
*********************************************************************/
...
#define Default_SendToUnity_getWs_SERIALIZE OM_NO_OP

#define Default_SendToUnity_setWs_SERIALIZE aomsmethod->addAttribute("p_ws", UNKNOWN2STRING(p_ws));
...
const WSADATA SendToUnity::getWs(void) const {
    NOTIFY_OPERATION(getWs, getWs() const, 0, Default_SendToUnity_getWs_SERIALIZE);
    //#[ operation getWs() const
    return ws;
    //#]
}

void SendToUnity::setWs(const WSADATA p_ws) {
    NOTIFY_OPERATION(setWs, setWs(const WSADATA), 1, Default_SendToUnity_setWs_SERIALIZE);
    //#[ operation setWs(WSADATA)
    ws = p_ws;
    //#]
}
